...
- **승인된 목업**: `design/weekly-report-mockup.html` — 순수 HTML/CSS/vanilla JS 자체완결 파일(Claude Artifact로 발행해 검토 완료, 여러 차례 피드백 반영). 실제 데이터 모델(프로젝트/티켓/구분/시간/일수/완료율/개발완료일·테스트예정일·배포예정일/비고/맨위크/이월)을 그대로 반영하되 화면 구조를 **대시보드 랜딩페이지 없이 "작성" / "히스토리" 2탭으로 단순화**함 — 기존 3화면(대시보드/등록/이력) 구조를 대체. 이 파일이 색상 토큰·컴포넌트 스타일·IA의 기준(source of truth)임.
  ...
  - **프로젝트 그룹 입력 구조**: "프로젝트 → 항목(플랫)" 대신 **"프로젝트 → 일감(티켓) → 그 안에 세부 항목들"** 구조. 일감 블록의 첫 줄은 **일감번호 + 제목**. 세부 항목은 처음엔 "분석/설계·개발·테스트 3줄 고정"으로 시도했다가 사용자 피드백으로 **"그때그때 추가"** 방식으로 바뀜 — 구분(분석/설계/개발/테스트)은 각 항목마다 셀렉트로 고르고, **같은 구분을 여러 개 추가할 수 있음**(예: 한 티켓에 "로그인 기능"·"2차인증 기능" 처럼 개발 항목 2개). 세부 항목 모양은 사실상 기존 flat itemRow에서 티켓번호만 상위(일감 헤더)로 옮긴 것과 같음. **이건 순수 입력 UI 개편이고 데이터 스키마 변경은 아님** — md 내보내기 시에는 손댄 세부 항목마다 여전히 스키마의 flat한 `- {티켓} : {제목} [구분] — {시간}h · {완료율}%` 한 줄씩으로 풀어서 나감(제목은 세부 항목 제목이 있으면 그것, 없으면 일감 제목). 즉 백엔드 `ReportItem`(항목 1개 = 줄 1개) 모델은 그대로 두고, Thymeleaf 이식 시 "같은 티켓번호를 가진 project 항목들을 화면에서만 일감 단위로 묶어 보여주는" 프레젠테이션 레이어만 추가하면 됨.
