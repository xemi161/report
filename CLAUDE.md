...
## 다음에 할 일 (2026-08-05 기준 미결)

- **`.md` 내보내기의 `<!--DATA...DATA-->` JSON 블록 제거 요청**: 사용자가 명시적으로 "json은 배제하고 최대한 마크다운을 틀에 맞춰서 파싱하는 형태로 가야할 거 같다"고 결정함. 아직 착수 전. 진행 시 손댈 곳:
  - `MdExportService.java` — JSON 직렬화 로직 제거, 파싱은 마크다운 본문 구조(고정 그룹 순서 프로젝트→개발→기타→휴가, `- {티켓} : {제목} [구분] — {시간}h · {완료율}%` 형태 라인)를 그대로 읽어들이는 방식으로 전환
  - `docs/weekly-report-md-schema.md` — JSON 블록을 정의한 부분을 마크다운 전용 파싱 규칙으로 갱신
  - 이 변경은 파트장/팀장 프로그램(아직 미개발, 범위 밖)이 나중에 이 `.md`를 파싱할 때의 계약을 바꾸는 아키텍처 결정이므로, 코드 작업 전 정확한 마크다운 문법(각 줄 포맷, 구분자, 필수/선택 필드 표기)을 사용자와 한 번 더 확인하는 게 안전함.
- **개발 그룹의 티켓번호 필수 여부**: 현재 스키마 검증은 project/dev 그룹 모두 티켓번호 필수라 없으면 제출이 막힘. 그런데 승인된 목업엔 티켓 없는 개발 항목("사내 위키 정리") 예시가 있었고 md 포맷 자체도 티켓 없는 줄을 지원함. dev 그룹은 티켓을 선택(optional)으로 풀지 결정 필요 (`docs/2026-08-02-frontend-port-worklog.md` §5).
- **`Project.active`(프로젝트 종료) 집계/표시 방식 미정**: 현재는 카드 삭제 시 비활성 처리에만 쓰고 있음. 종료된 프로젝트를 히스토리·통계에 어떻게 반영할지 결정 필요.
- **`docs/project-handoff-summary.md` 자체는 아직 옛 "제출 후 고정" 규칙을 담고 있어 실제 동작과 어긋남** — 문서를 현재 정책에 맞게 고칠지 결정 필요 (현재는 CLAUDE.md의 각주로만 "폐기됨"을 표시해둔 상태).
